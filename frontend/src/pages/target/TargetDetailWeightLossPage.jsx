...
export default function TargetDetailWeightLossPage() {
  ...
  useEffect(() => {
    if (user_info) {
      ...
      setDescription(target.loss_weight.description || "");
      setTimeStart(
        target.loss_weight.time_start
          ? moment(target.loss_weight.time_start)
          : null
      );
      setTimeEnd(
        target.loss_weight.time_end ? moment(target.loss_weight.time_end) : null
      );
      setInitWeight(target.loss_weight.init_w || 0);
      setTargetWeight(target.loss_weight.target_w || 0);
      setCurrWeight(target.loss_weight.crr_w || 0);
    }
  }, [user_info]);
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.put(
        `/api/update_loss_weight/${user_info._id}`,
        payload
      );
      ...
      {user_info ? (
        <>
          ...
          <div className="w-1/2 flex flex-row gap-6 px-6 mb-6">
            <LocalizationProvider dateAdapter={AdapterMoment}>
              <DemoContainer components={["DateTimePicker"]}>
                <DateTimePicker
                  label={t("loss_weight.time_start")}
                  value={timeStart}
          ...
          <div className="w-1/2 flex flex-row gap-6 px-6 mb-6">
            <TextField
              type="number"
              label={t("loss_weight.init_weight")}
              value={initWeight}
              onChange={(e) => setInitWeight(e.target.value)}
            />
            <TextField
              type="number"
              label={t("loss_weight.target_end")}
              value={targetWeight}
          ...
          <div className="w-1/2 flex flex-row gap-6 px-6 mb-24">
            <TextField
              type="number"
              label={t("loss_weight.curr_weight")}
              value={currWeight}
        ...
        <p>{t("loss_weight.no_user_info")}</p>
      )}
    </div>
  );
}
